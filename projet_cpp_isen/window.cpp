...
void Window::valider() {
    if (_elements.size() != 0){
        ...
        // Notation classique
        QString cla = QString::fromStdString(_interactionUtilisateur->getExpression()->affichageClassique());
        _classique->setVisible(true);
        _classique->setText(cla);

        // Notation polonaise inversée
        QString pol = QString::fromStdString(_interactionUtilisateur->getExpression()->affichagePolonaiseInversee());
